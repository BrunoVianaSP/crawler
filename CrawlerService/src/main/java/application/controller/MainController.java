...
public class MainController {
	...
	public @ResponseBody ResponseEntity<Feed> getFeed(@PathVariable("user") String user,
		...
		try {
			...
			BufferedReader oReader = requestXMLPage();
	...
	private BufferedReader requestXMLPage() throws MalformedURLException, IOException {
...
// InputStreamReader(oConnection.getInputStream()));
// Feed feed = service.createFeed(oReader);
// return new ResponseEntity<Feed>(feed, HttpStatus.OK);
// } catch (IllegalArgumentException e) {
// return new ResponseEntity<Feed>(new Feed(), HttpStatus.FORBIDDEN);
// } catch (Exception e) {
// e.printStackTrace();
// return new ResponseEntity<Feed>(new Feed(),
